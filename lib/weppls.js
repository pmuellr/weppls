...
var Program, Verbose, Version, coffee, copyResources, createFilters, createIndexHtml, createIndexScript, createServices, createViews, iSubDirs, main, marked, path, pkg, sh, weppls, _,
...
createServices = function(iDir, oDir, options) {
  var base, cFiles, content, ext, file, files, fullFile, jFileLen, jFiles, match, name, oFile, servicesDir, _i, _len;
  ...
  jFileLen = (_.max(jFiles, function(jFile) {
    return jFile.length;
  })).length;
  content = _.map(jFiles, function(jFile) {
    var pad;
    pad = weppls.align.left("", jFileLen - jFile.length);
    return "    angularModule.service('" + jFile + "'" + pad + ", require('./services/" + jFile + "'" + pad + ").service);";
  });
...
createFilters = function(iDir, oDir, options) {
  var base, cFiles, content, ext, file, files, filtersDir, fullFile, jFileLen, jFiles, match, name, oFile, _i, _len;
  ...
  content = _.map(jFiles, function(jFile) {
    var pad;
    pad = weppls.align.left("", jFileLen - jFile.length);
    return "    angularModule.filter('" + jFile + "'" + pad + ", require('./filters/" + jFile + "'" + pad + ").filter);";
  });
  content = "// generated on " + (weppls.getDate()) + " by " + Program + " " + Version + "\n\nexports.configure = function(angularModule) {\n" + (content.join('\n')) + "\n};";
